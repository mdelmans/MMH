...
const char* g_DeviceNameArduinoXYStage = "Arduino-XYStage"; 
...
MODULE_API void InitializeModuleData()
{
   ...
   RegisterDevice(g_DeviceNameArduinoXYStage, MM::XYStageDevice, "XY Stage");	
}
...
MODULE_API MM::Device* CreateDevice(const char* deviceName)
{
   ...
   else if (strcmp(deviceName, g_DeviceNameArduinoXYStage) == 0)
   {
      return new CArduinoXYStage;
   }
...
int CArduinoHub::DetectInstalledDevices()
{
   if (MM::CanCommunicate == DetectDevice()) 
   {
	  ...
	  peripherals.push_back(g_DeviceNameArduinoXYStage);
...
///////////////////////////////////////////////////////////////////////////////
// CArduinoXYStage implementation
// ~~~~~~~~~~~~~~~~~~~~~~~~~~

CArduinoXYStage::CArduinoXYStage():
	initialized_(false),
	...
	maxY_(1000)
{
	...
	SetErrorText(ERR_COMMUNICATION, "Error in communication with Arduino board");
	SetErrorText(ERR_NO_PORT_SET, "Hub Device not found.  The Arduino Hub device is needed to create this device");
	SetErrorText(ERR_STOP_COMMAND_ISSUED, "Stop command was issued");
...
int CArduinoXYStage::Initialize()
{

	SetErrorText(ERR_REACHED_STAGE_LIMIT, "Warning! Stage has reached the physical limit.");
...
int CArduinoXYStage::Shutdown()
{
	initialized_ = false;
	return DEVICE_OK;
}

void CArduinoXYStage::GetName(char* name) const
{
	CDeviceUtils::CopyLimitedString(name, g_DeviceNameArduinoXYStage);
}

int CArduinoXYStage::SetPositionUm(double x, double y)
{
	long stepsX, stepsY;
	...
	return SetPositionSteps(stepsX, stepsY);

}

int CArduinoXYStage::SetRelativePositionUm(double dx, double dy)
{
	long dStepsX, dStepsY;

	dStepsX = dx / stepSizeX_;
	dStepsY = dy / stepSizeY_;

	return SetRelativePositionSteps(dStepsX, dStepsY);
}

int CArduinoXYStage::GetPositionUm(double& x, double& y)
{
	
	x = stepSizeX_ * stepsX_;
	y = stepSizeY_ * stepsY_;

	return DEVICE_OK;
}

int CArduinoXYStage::SetPositionSteps(long x, long y)
{

	long dStepsX = x - stepsX_;
	long dStepsY = y - stepsY_;

	return SetRelativePositionSteps(dStepsX, dStepsY);
}


int CArduinoXYStage::SetRelativePositionSteps(long dX , long dY)
{
	busy_=true;
	...
	command[0] = 51;
	command[1] = (unsigned char)(idX & 0xFF);
	command[2] = (unsigned char)((idX >> 8) & 0xFF);
	command[3] = (unsigned char)(idY & 0xFF);
	command[4] = (unsigned char)((idY >> 8) & 0xFF);

	int ret = hub->WriteToComPortH((const unsigned char*) command, 5);
	if (ret != DEVICE_OK)
		return ret;
	...
	if (answer[0] == 2)
		return ERR_REACHED_STAGE_LIMIT;
...
int CArduinoXYStage::GetPositionSteps(long& x, long& y)
{ 
	x = stepsX_; 
	y = stepsY_; 
...
int CArduinoXYStage::SetAdapterOriginUm(double x, double y)
{
	long originX = x / stepSizeX_;
	long originY = y / stepSizeY_;
...
int CArduinoXYStage::Stop()
{
	stop_ = true;
	return DEVICE_OK;
}

int CArduinoXYStage::IsXYStageSequenceable(bool& isSequenceable) const
{
...
int CArduinoXYStage::GetLimitsUm(double& xMin, double& xMax, double& yMin, double& yMax)
{
	xMin = -maxX_ * stepSizeX_;
	yMin = -maxY_ * stepSizeY_;
	xMax = maxX_ * stepSizeX_;
	yMax = maxY_ * stepSizeY_;

	return DEVICE_OK;
}

int CArduinoXYStage::GetStepLimits(long& xMin, long& xMax, long& yMin, long& yMax)
{
	xMin = -maxX_;
	yMin = -maxY_;
	xMax = maxX_;
...
int CArduinoXYStage::Home()
{
	int ret = SetPositionSteps(0,0);
	return ret;
}